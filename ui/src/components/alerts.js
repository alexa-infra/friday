...
import { dismiss, Alerts } from '../slices';

const Alert = ({ item, onDismissAlert }) => {
  const { message, type } = item;
  return (
    <div
      className={classNames('rounded my-1 py-2 px-4 flex', {
    ...
    >
      ...
      <button
        type="button"
        className="cursor-pointer text-white bg-transparent opacity-50 hover:opacity-75"
...
export const AlertList = () => {
  ...
  return (
    <div className="flex flex-col flex-nowrap">
      {items.map((it) => (
        <Alert key={it.id} item={it} onDismissAlert={onDismissAlert} />
      ))}
    </div>
  );
};
